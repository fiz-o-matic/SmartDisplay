...
void can_init () {
        ...
        if (errorCode == 0) {
                DEBUG_PRINTLN ("Bit Rate prescaler:  " + String(settings.mBitRatePrescaler)) ;
                DEBUG_PRINTLN ("Propagation Segment: " + String(settings.mPropagationSegment)) ;
                DEBUG_PRINTLN ("Phase Segment 1:     " + String(settings.mPhaseSegment1)) ;
                DEBUG_PRINTLN ("Phase Segment 2:     " + String(settings.mPhaseSegment2)) ;
                DEBUG_PRINTLN ("SJW:                 " + String(settings.mSJW)) ;
                DEBUG_PRINTLN ("Triple Sampling:     " + String(settings.mTripleSampling ? "yes" : "no")) ;
                DEBUG_PRINTLN ("Actual bit rate:     " + String(settings.actualBitRate()) + " bit/s") ;
                DEBUG_PRINTLN ("Exact bit rate ?     " + String(settings.exactBitRate () ? "yes" : "no")) ;
                DEBUG_PRINTLN ("Distance             " + String(settings.ppmFromDesiredBitRate()) + " ppm") ;
                DEBUG_PRINTLN ("Sample point:        " + String(settings.samplePointFromBitStart()) + "%") ;
                DEBUG_PRINTLN ("Configuration OK!");
        }else{
                //Serial.print ("Configuration error 0x") ;
                //Serial.println (errorCode, HEX) ;
                DEBUG_PRINTLN ("Configuration error 0x" + String(errorCode, HEX)) ;
        }
...
static void read(CANMessage frame) {
    ...
    switch ( frame.id ) {
        case DISPLAY_ADDR + 0x01:
            ...
            DEBUG_PRINTLN("receive display description for 0x" + String(frame.data16[0], HEX));
